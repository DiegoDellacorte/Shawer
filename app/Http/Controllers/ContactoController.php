...
class ContactoController extends Controller
{
    ...
    public function presupuesto( Request $request ) {
        ...
        foreach ($prods_form as $prod) {
            ...
            Mail::to('soporte@osole.es')->send( new Pedido( $dataRequest,$productos ) );
            $obj= new \stdClass();
            $obj->respuesta=false;
    ...
    public function enviarConsulta(Request $request){
        ...
        if (count(Mail::failures()) > 0){
            $obj->respuesta=true;
            return back()->with('error',"Hubo un problema.Intente más tarde");
        }else{
            return back()->with('success',"Su consulta ha sido recibida");
        }
    }
    ...
    public function vistaPresupuestoProducto($id){
